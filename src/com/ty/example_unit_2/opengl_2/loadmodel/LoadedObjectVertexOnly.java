...
public class LoadedObjectVertexOnly {
	...
	public LoadedObjectVertexOnly(ModelView mv, float[] vertices,float[] textureCoors,float[] normalCoors,Vector3 centerPosition) {
	 ...
	 //初始化顶点坐标的方法
    public void initVertexData(float[] vertices,float[] textureCoor,float [] normalCoor)
    {
    	//顶点坐标数据的初始化================begin============================
    ...
    {
    	//加载顶点着色器的脚本内容
        mVertexShader=ShaderUtil.loadFromAssetsFile("data/unit2/shader/vertex.sh", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("data/unit2/shader/frag.sh", mv.getResources());  
        //基于顶点着色器与片元着色器创建程序
