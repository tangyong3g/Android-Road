...
public class CatmullRomSpline<T extends Vector<T>> implements Path<T> {
	 ...
	 */
	public static <T extends Vector<T>> T calculate(final T out, final float t,
			final T[] points, final boolean continuous, final T tmp) {
	 ...
	 * @return The value of out
	 */
	public static <T extends Vector<T>> T calculate(final T out, final int i,
			final float u, final T[] points, final boolean continuous,
			final T tmp) {
		...
		if (continuous || i > 0)
			out.add(tmp.set(points[(n + i - 1) % n]).scl(
					-0.5f * u3 + u2 - 0.5f * u));
		if (continuous || i < (n - 1))
			out.add(tmp.set(points[(i + 1) % n]).scl(
	...
	public CatmullRomSpline set(final T[] controlPoints,
	...
	/** @return The value of the c at position u of the specified span */
	public T valueAt(final T out, final int span, final float u) {
		return calculate(out, continuous ? span : (span + 1), u, controlPoints,
				continuous, tmp);
	}
