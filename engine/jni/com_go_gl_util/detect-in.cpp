...
//NDK函数的实现
void Java_com_go_gl_util_NdkUtil_detectGLES20
...
//		 {"detectGLES20", "(Landroid/content/Context;)V", (void*)Java_com_go_gl_util_NdkUtil_detectGLES20},//绑定
//};
...
static jboolean checkSignature(JNIEnv * env, jobject context) {
	...
	jclass cls_GLActivity = env->FindClass("com/go/gl/GLActivity");
	CHK_RET(cls_GLActivity, false);
	jmethodID mthd_getContentGlView = env->GetMethodID(cls_GLActivity, "getContentGlView",
			"()Lcom/go/gl/view/GLView;");
	...
	jmethodID mthd_Context_getPackageName = env->GetMethodID(cls_Context, "getPackageName",
			"()Ljava/lang/String;");
...
//	printString(env, pkgName, "pkgName");
	...
	jmethodID mthd_Context_getPackageManager = env->GetMethodID(cls_Context, "getPackageManager",
	...
	jclass cls_PackageManager = env->FindClass("android/content/pm/PackageManager");	//abstract class, shouldn't use getObjectClass()
	CHK_RET(cls_PackageManager, false);

	/*
	//int GET_SIGNATURES = PackageManager.GET_SIGNATURES;
	jfieldID fld_PackageManager_GET_SIGNATURES = env->GetStaticFieldID(cls_PackageManager, "GET_SIGNATURES",
			"I");
	...
	LOGI("GET_SIGNATURES=0x%08X", GET_SIGNATURES);
	*/
	jint GET_SIGNATURES = 0x00000040;

	//PakageInfo pkgInfo = pkgMgr.getPackageInfo(pkgName, PackageManager.GET_SIGNATURES);
	jmethodID mthd_PackageManager_getPackageInfo = env->GetMethodID(cls_PackageManager, "getPackageInfo",
	...
	jfieldID fld_PackageInfo_signatures = env->GetFieldID(cls_PackageInfo, "signatures",
	...
	jmethodID mthd_Signature_toCharsString = env->GetMethodID(cls_Signature, "toCharsString",
			"()Ljava/lang/String;");
...
//	printString(env, signStr, "sign");
	...
	jmethodID mthd_MessageDigest_getInstance = env->GetStaticMethodID(cls_MessageDigest, "getInstance",
			"(Ljava/lang/String;)Ljava/security/MessageDigest;");
	CHK_RET(mthd_MessageDigest_getInstance, false);
	jstring algoStr = env->NewStringUTF("MD5");
	...
	jmethodID mthd_MessageDigest_reset = env->GetMethodID(cls_MessageDigest, "reset",
			"()V");
	...
	jmethodID mthd_String_getBytes = env->GetMethodID(cls_String, "getBytes",
			"()[B");
	...
	jmethodID mthd_MessageDigest_update = env->GetMethodID(cls_MessageDigest, "update",
			"([B)V");
	...
	jmethodID mthd_MessageDigest_digest = env->GetMethodID(cls_MessageDigest, "digest",
			"()[B");
	...
	jclass cls_Arrays = env->FindClass("java/util/Arrays");
	CHK_RET(cls_Arrays, false);
	jmethodID mthd_Arrays_equals = env->GetStaticMethodID(cls_Arrays, "equals",
			"([B[B)Z");
...
//	CHK_RET(signMatch, false);
	...
	jmethodID mthd_setVisibility = env->GetMethodID(cls_GLView, "setVisibility",
			"(I)V");
	...
	if (maskedNumber == randomNumber) {
		...
		jmethodID mthd_setContentGlView = env->GetMethodID(cls_GLActivity,
				"setContentGlView", "(Lcom/go/gl/view/GLView;)V");
