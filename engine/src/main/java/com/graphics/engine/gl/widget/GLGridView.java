...
public class GLGridView extends GLAbsListView {
    ...
    private GLView mReferenceView = null;
    private GLView mReferenceViewInSelectedRow = null;
    ...
    public GLGridView(Context context, AttributeSet attrs, int defStyle) {
        ...
        int hSpacing = a.getDimensionPixelOffset(
                com_android_internal_R_styleable.GridView_horizontalSpacing, 0);
        setHorizontalSpacing(hSpacing);

        int vSpacing = a.getDimensionPixelOffset(
                com_android_internal_R_styleable.GridView_verticalSpacing, 0);
        setVerticalSpacing(vSpacing);

        int index = a.getInt(com_android_internal_R_styleable.GridView_stretchMode, STRETCH_COLUMN_WIDTH);
        if (index >= 0) {
            setStretchMode(index);
        }

        int columnWidth = a.getDimensionPixelOffset(com_android_internal_R_styleable.GridView_columnWidth, -1);
        if (columnWidth > 0) {
            setColumnWidth(columnWidth);
        }

        int numColumns = a.getInt(com_android_internal_R_styleable.GridView_numColumns, 1);
        setNumColumns(numColumns);

        index = a.getInt(com_android_internal_R_styleable.GridView_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }
        ...
        //TODO:remove this;Currently in GLView's draw(),didn't support FadingEdgeEnabled set to true
    ...
    /**
     * Sets the data behind this GLBaseGridView.
     *
    ...
    private GLView fillDown(int pos, int nextTop) {
        ...
        while (nextTop < end && pos < mItemCount) {
            GLView temp = makeRow(pos, nextTop, true);
            if (temp != null) {
                selectedView = temp;
            }
    ...
    private GLView makeRow(int startPos, int y, boolean flow) {
    ...
    private GLView fillUp(int pos, int nextBottom) {
        ...
        while (nextBottom > end && pos >= 0) {
            ...
            GLView temp = makeRow(pos, nextBottom, false);
            if (temp != null) {
                selectedView = temp;
            }
    ...
    private GLView fillFromBottom(int lastPosition, int nextBottom) {
    ...
    private GLView fillSelection(int childrenTop, int childrenBottom) {
        ...
        final GLView sel = makeRow(mStackFromBottom ? rowEnd : rowStart, topSelectionPixel, true);
        mFirstPosition = rowStart;

        final GLView referenceView = mReferenceView;
    ...
    private GLView fillSpecific(int position, int top) {
        ...
        final GLView temp = makeRow(mStackFromBottom ? motionRowEnd : motionRowStart, top, true);
    ...
    private void correctTooHigh(int numColumns, int verticalSpacing, int childCount) {
        ...
        if (lastPosition == mItemCount - 1 && childCount > 0) {
            ...
            // This is how far the bottom edge of the last GLView is from the bottom of the
            // drawable area
    ...
    private void correctTooLow(int numColumns, int verticalSpacing, int childCount) {
        if (mFirstPosition == 0 && childCount > 0) {
            ...
            // This is how far the top edge of the first GLView is from the top of the
            // drawable area
     ...
     * @return The GLView that currently has selection
     */
     ...
     *        selection into
     */
    private void adjustForBottomFadingEdge(GLView childInSelectedRow,
        ...
        if (childInSelectedRow.getBottom() > bottomSelectionPixel) {
            ...
            // Now offset the selected item to get it into GLView
            offsetChildrenTopAndBottom(-offset);
        }
    }
     ...
     *        selection into
     */
    private void adjustForTopFadingEdge(GLView childInSelectedRow,
            int topSelectionPixel, int bottomSelectionPixel) {
     ...
     * @return The GLView that currently has selection
     */
    private GLView moveSelection(int delta, int childrenTop, int childrenBottom) {
        final int fadingEdgeLength = getVerticalFadingEdgeLength();
        final int selectedPosition = mSelectedPosition;
        final int numColumns = mNumColumns;
        final int verticalSpacing = mVerticalSpacing;
...
//    }
    ...
    protected void layoutChildren() {
        ...
        try {
            ...
            case LAYOUT_MOVE_SELECTION:
                ...
                break;
            default:
                // Remember the previously selected GLView
    ...
    /**
     * Obtain the GLView and add it to our list of children. The GLView can be made
     * fresh, converted from an unused GLView, or used as is if it was in the
     * recycle bin.
     *
     * @param position Logical position in the list
     * @param y Top or bottom edge of the GLView to add
    ...
    private GLView makeAndAddView(int position, int y, boolean flow, int childrenLeft,
        ...
        if (!mDataChanged) {
            // Try to use an existing GLView for this position
        ...
        // Make a new GLView for this position, or convert an unused GLView if
        // possible
    ...
    /**
     * Add a GLView as a child and make sure it is measured (if necessary) and
     * positioned properly.
     *
     * @param child The GLView to add
     * @param position The position of the GLView
     * @param y The y position relative to which this GLView will be positioned
     * @param flow if true, align top edge to y. If false, align bottom edge
     *        to y.
     * @param childrenLeft Left edge where children should be positioned
     * @param selected Is this position selected?
     * @param recycled Has this GLView been pulled from the recycle bin? If so it
     *        does not need to be remeasured.
     * @param where Where to add the item in the list
     *
     */
    private void setupChild(GLView child, int position, int y, boolean flow, int childrenLeft,
        ...
        // Respect layout params that are already in the GLView. Otherwise make
        // some up...
		GLAbsListView.LayoutParams p = (GLAbsListView.LayoutParams) child.getLayoutParams();
        if (p == null) {
            p = new GLAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
...
//        }
     ...
     *
     * @param direction either {@link GLView#FOCUS_UP} or {@link GLView#FOCUS_DOWN}
     ...
     *
     * @param direction either {@link GLView#FOCUS_LEFT}, {@link GLView#FOCUS_RIGHT},
     *        {@link GLView#FOCUS_UP} or {@link GLView#FOCUS_DOWN}
     *
     * @return whether selection was moved
     */
    ...
    protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        ...
        if (gainFocus && previouslyFocusedRect != null) {
            ...
            final int childCount = getChildCount();
            for (int i = 0; i < childCount; i++) {
                // only consider GLView's on appropriate edge of grid
                if (!isCandidateSelection(i, direction)) {
                    continue;
                }

                final GLView other = getChildAt(i);
    ...
    protected int computeVerticalScrollExtent() {
        ...
        if (count > 0) {
            ...
            int extent = rowCount * 100;	// CHECKSTYLE IGNORE
    ...
    protected int computeVerticalScrollRange() {
        final int numColumns = mNumColumns;
        final int rowCount = (mItemCount + numColumns - 1) / numColumns;
        return Math.max(rowCount * 100, 0);	// CHECKSTYLE IGNORE
    }
}
