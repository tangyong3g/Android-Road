...
public class AnimationSet extends Animation {
    ...
    private Transformation3D mTempTransformation = new Transformation3D();
...
//        a.recycle();
    ...
    protected AnimationSet clone() throws CloneNotSupportedException {
        ...
        animation.mTempTransformation = new Transformation3D();
     ...
     */
    @Override
    public boolean getTransformation(long currentTime, Transformation3D t) {
		mCurrentTime = currentTime;
		if (mDuration != 0) {
			mNormalizeTime = (currentTime - (mStartTime + mStartOffset)) / (float) mDuration;
		} else {
			mNormalizeTime = currentTime < mStartTime ? 0.0f : 1.0f;
		}
		mNormalizeTime = Math.max(Math.min(mNormalizeTime, 1.0f), 0.0f);
		if (mCycleFlip) {
			mNormalizeTime = 1.0f - mNormalizeTime;
		}
