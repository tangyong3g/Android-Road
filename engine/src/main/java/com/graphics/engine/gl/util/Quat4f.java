 ...
 * <br>类描述: 四元数类
 * <br>功能详细描述:
 * @deprecated 使用{@link com.graphics.engine.gl.math3d.Quaternion}代替
 */
public class Quat4f {
	...
	public final void set(Vector3f euler) {
		...
		angle = euler.z * 0.5f;	// CHECKSTYLE IGNORE
		sy = (float) Math.sin(angle);
		cy = (float) Math.cos(angle);
		angle = euler.y * 0.5f;	// CHECKSTYLE IGNORE
		sp = (float) Math.sin(angle);
		cp = (float) Math.cos(angle);
		angle = euler.x * 0.5f;	// CHECKSTYLE IGNORE
	...
	public final void interpolate(Quat4f q1, Quat4f q2, float alpha) {
		...
		if ((1.0 - dot) > 0.000001f) {	// CHECKSTYLE IGNORE
			om = Math.acos(dot);
			sinom = Math.sin(om);
