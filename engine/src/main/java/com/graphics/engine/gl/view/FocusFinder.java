...
public class FocusFinder {
    ...
    private static ThreadLocal<FocusFinder> tlFocusFinder =              //CHECKSTYLE IGNORE
     ...
     */
    public final GLView findNextFocus(GLViewGroup root, GLView focused, int direction) {

        if (focused != null) {
            // check for user specified next focus
            GLView userSetNextFocus = focused.findUserSetNextFocus(root, direction);
     ...
     * @return The next focusable view, or null if none exists.
     */
    public GLView findNextFocusFromRect(GLViewGroup root, Rect focusedRect, int direction) {
        return findNextFocus(root, null, focusedRect, direction);
    }

    private GLView findNextFocus(GLViewGroup root, GLView focused, Rect focusedRect, int direction) {
        ArrayList<GLView> focusables = root.getFocusables(direction);
        ...
        for (int i = 0; i < numFocusables; i++) {
            GLView focusable = focusables.get(i);
     ...
     */
    public GLView findNearestTouchable(GLViewGroup root, int x, int y, int direction, int[] deltas) {
        ArrayList<GLView> touchables = root.getTouchables();
        ...
        for (int i = 0; i < numTouchables; i++) {
            GLView touchable = touchables.get(i);
