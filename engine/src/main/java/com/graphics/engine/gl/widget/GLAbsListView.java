 ...
 */
public abstract class GLAbsListView extends GLAdapterView<GLListAdapter> implements
...
//    EditText mTextFilter;
     ...
     * Acts upon click
     */
    private GLAbsListView.PerformClick mPerformClick;
...
//    private boolean mPopupHidden;
    ...
    public GLAbsListView(Context context) {
        ...
        TypedArray a = context.obtainStyledAttributes(com_android_internal_R_styleable.View);
        initializeScrollbars(a);
        a.recycle();
    }

    public GLAbsListView(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.absListViewStyle);
    }

    public GLAbsListView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initAbsListView();

        TypedArray a = context.obtainStyledAttributes(attrs,
                com_android_internal_R_styleable.AbsListView, defStyle, 0);

        Drawable d = a.getDrawable(com_android_internal_R_styleable.AbsListView_listSelector);
        if (d != null) {
            setSelector(d);
        }

        mDrawSelectorOnTop = a.getBoolean(
                com_android_internal_R_styleable.AbsListView_drawSelectorOnTop, false);

        boolean stackFromBottom = a.getBoolean(com_android_internal_R_styleable.AbsListView_stackFromBottom, false);
        setStackFromBottom(stackFromBottom);

//        boolean scrollingCacheEnabled = a.getBoolean(R.styleable.AbsListView_scrollingCache, true);
//        setScrollingCacheEnabled(scrollingCacheEnabled);

        boolean useTextFilter = a.getBoolean(com_android_internal_R_styleable.AbsListView_textFilterEnabled, false);
        setTextFilterEnabled(useTextFilter);

        int transcriptMode = a.getInt(com_android_internal_R_styleable.AbsListView_transcriptMode,
                TRANSCRIPT_MODE_DISABLED);
        setTranscriptMode(transcriptMode);

        int color = a.getColor(com_android_internal_R_styleable.AbsListView_cacheColorHint, 0);
        setCacheColorHint(color);

        boolean smoothScrollbar = a.getBoolean(com_android_internal_R_styleable.AbsListView_smoothScrollbar, true);
...
//    }
    ...
    public void setScrollingCacheEnabled(boolean enabled) {
    	//TODO: 可以使用，但是反而慢了，而且有时绘制会不正确裁剪了
...
//        mScrollingCacheEnabled = enabled;
    ...
    static class SavedState extends BaseSavedState {	// CHECKSTYLE IGNORE
        ...
        // CHECKSTYLE:ON
        /**
         * Constructor called from {@link GLAbsListView#onSaveInstanceState()}
         */
        ...
        public String toString() {
            return "GLAbsListView.SavedState{"
...
//    }
    ...
    protected int computeVerticalScrollExtent() {
        ...
        if (count > 0) {
            if (mSmoothScrollbarEnabled) {
                int extent = count * 100;				// CHECKSTYLE IGNORE

                GLView view = getChildAt(0);
                final int top = view.getTop();
                int height = view.getHeight();
                if (height > 0) {
                    extent += (top * 100) / height;		// CHECKSTYLE IGNORE
                }
                ...
                if (height > 0) {
                    extent -= ((bottom - getHeight()) * 100) / height;	// CHECKSTYLE IGNORE
                }
    ...
    protected int computeVerticalScrollOffset() {
        ...
        if (firstPosition >= 0 && childCount > 0) {
            if (mSmoothScrollbarEnabled) {
                final GLView view = getChildAt(0);
                final int top = view.getTop();
                int height = view.getHeight();
                if (height > 0) {
					return Math.max(firstPosition * 100 - (top * 100) / height					// CHECKSTYLE IGNORE
							+ (int) ((float) mScrollY / getHeight() * mItemCount * 100), 0);	// CHECKSTYLE IGNORE
                }
            } else {
                int index;
    ...
    protected int computeVerticalScrollRange() {
        int result;
        if (mSmoothScrollbarEnabled) {
            result = Math.max(mItemCount * 100, 0);	// CHECKSTYLE IGNORE
        } else {
            result = mItemCount;
        }
        return result;
...
//        }
    ...
    void updateScrollIndicators() {
        ...
        if (mScrollDown != null) {
            ...
            if (!canScrollDown && count > 0) {
                GLView child = getChildAt(count - 1);
     ...
     */
    GLView obtainView(int position, boolean[] isScrap) {
        isScrap[0] = false;
        GLView scrapView;
...
//            }
    ...
    void positionSelector(GLView sel) {
...
//            selector.draw(canvas);
    ...
    void keyPressed() {
        ...
        if (selector != null && (isFocused() || touchModeDrawsInPressedState())
            ...
            final GLView v = getChildAt(mSelectedPosition - mFirstPosition);
...
//            }
     ...
     */
    ContextMenuInfo createContextMenuInfo(GLView view, int position, long id) {
    ...
    private class PerformClick extends WindowRunnnable implements Runnable {	// CHECKSTYLE IGNORE
        GLView mChild;
    ...
    private class CheckForLongPress extends WindowRunnnable implements Runnable {	// CHECKSTYLE IGNORE
        public void run() {
            final int motionPosition = mMotionPosition;
            final GLView child = getChildAt(motionPosition - mFirstPosition);
            if (child != null) {
    ...
    private class CheckForKeyLongPress extends WindowRunnnable implements Runnable {	// CHECKSTYLE IGNORE
        public void run() {
            if (isPressed() && mSelectedPosition >= 0) {
                int index = mSelectedPosition - mFirstPosition;
                GLView v = getChildAt(index);
    ...
    private boolean performLongPress(final GLView child,
            final int longPressPosition, final long longPressId) {
        boolean handled = false;

        if (mOnItemLongClickListener != null) {
            handled = mOnItemLongClickListener.onItemLongClick(GLAbsListView.this, child,
                    longPressPosition, longPressId);
        }
        if (!handled) {
            mContextMenuInfo = createContextMenuInfo(child, longPressPosition, longPressId);
            handled = super.showContextMenuForChild(GLAbsListView.this);
        }
        if (handled) {
    ...
    public boolean showContextMenuForChild(GLView originalView) {
        ...
        if (longPressPosition >= 0) {
            ...
            if (mOnItemLongClickListener != null) {
                handled = mOnItemLongClickListener.onItemLongClick(GLAbsListView.this, originalView,
                        longPressPosition, longPressId);
            }
            if (!handled) {
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        case KeyEvent.KEYCODE_ENTER:
            ...
            if (isClickable() && isPressed() &&
                ...
                final GLView view = getChildAt(mSelectedPosition - mFirstPosition);
                if (view != null) {
    ...
    final class CheckForTap implements Runnable {	// CHECKSTYLE IGNORE
        public void run() {
            if (mTouchMode == TOUCH_MODE_DOWN) {
                mTouchMode = TOUCH_MODE_TAP;
                final GLView child = getChildAt(mMotionPosition - mFirstPosition);
    ...
    private boolean startScrollIfNeeded(int deltaY) {
        ...
        if (distance > mTouchSlop) {
            ...
            // Handler should not be null unless the GLAbsListView is not attached to a
            ...
            setPressed(false);
            GLView motionView = getChildAt(mMotionPosition - mFirstPosition);
            if (motionView != null) {
    ...
    public void onTouchModeChanged(boolean isInTouchMode) {
        if (isInTouchMode) {
            ...
            if (getHeight() > 0 && getChildCount() > 0) {
                // We do not lose focus initiating a touch (since GLAbsListView is focusable in
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (!isEnabled()) {
            // A disabled GLView that is clickable still consumes the touch
...
//        }
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            if (!mDataChanged) {
                if ((mTouchMode != TOUCH_MODE_FLING) && (motionPosition >= 0)
                    ...
                    // User clicked on an actual GLView (and was not stopping a fling). It might be a
                    ...
                    if (ev.getEdgeFlags() != 0 && motionPosition < 0) {
                        // If we couldn't find a GLView to click on, but the down event was touching
                        // the edge, we will bail out and try again. This allows the edge correcting
                        // code in ViewRoot to try to find a nearby GLView to select
                        return false;
                    }
            ...
            case TOUCH_MODE_SCROLL:
                ...
                if (y != mLastY) {
                    ...
                    if (atEdge && getChildCount() > 0) {
                        ...
                        if (motionPosition >= 0) {
                            final GLView motionView = getChildAt(motionPosition - mFirstPosition);
        ...
        case MotionEvent.ACTION_UP: {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                if (child != null && !child.hasFocusable()) {
                    ...
                    final GLAbsListView.PerformClick performClick = mPerformClick;
            ...
            case TOUCH_MODE_SCROLL:
                ...
                if (childCount > 0) {
                    if (mFirstPosition == 0 && getChildAt(0).getTop() >= mListPadding.top &&
                        ...
                        velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);	// CHECKSTYLE IGNORE
        ...
        case MotionEvent.ACTION_CANCEL: {
            mTouchMode = TOUCH_MODE_REST;
            setPressed(false);
            GLView motionView = this.getChildAt(mMotionPosition - mFirstPosition);
            if (motionView != null) {
                motionView.setPressed(false);
            }
            clearScrollingCache();
...
//        }
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            if (touchMode != TOUCH_MODE_FLING && motionPosition >= 0) {
                // User clicked on an actual GLView (and was not stopping a fling).
                // Remember where the motion event started
                v = getChildAt(motionPosition - mFirstPosition);
                mMotionViewOriginalTop = v.getTop();
                mMotionX = x;
     ...
     * {@link android.widget.GLAbsListView.OnScrollListener}, if any. The state change
    ...
    private class FlingRunnable implements Runnable {
        ...
        public void run() {
            ...
            case TOUCH_MODE_FLING: {
                ...
                if (delta > 0) {
                    // List is moving towards the top. Use first GLView as mMotionPosition
                    mMotionPosition = mFirstPosition;
                    final GLView firstView = getChildAt(0);
                    mMotionViewOriginalTop = firstView.getTop();

                    // Don't fling more than 1 screen
                    delta = Math.min(getHeight() - mPaddingBottom - mPaddingTop - 1, delta);
                } else {
                    // List is moving towards the bottom. Use last GLView as mMotionPosition
                    int offsetToLast = getChildCount() - 1;
                    mMotionPosition = mFirstPosition + offsetToLast;

                    final GLView lastView = getChildAt(offsetToLast);
    ...
    class PositionScroller implements Runnable {		// CHECKSTYLE IGNORE
        ...
        public void run() {
            ...
            case MOVE_DOWN_POS: {
                ...
                final GLView lastView = getChildAt(lastViewIndex);
            ...
            case MOVE_DOWN_BOUND: {
                ...
                final GLView nextView = getChildAt(nextViewIndex);
    ...
    /**
     * Smoothly scroll to the specified adapter position. The GLView will
...
//                        }
    ...
    boolean resurrectSelection() {
        ...
        if (toPosition >= firstPosition && toPosition < firstPosition + childCount) {
            ...
            final GLView selected = getChildAt(selectedPos - mFirstPosition);
            ...
            if (toPosition < firstPosition) {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    if (top >= childrenTop) {
                        // Found a GLView whose top is fully visisble
    ...
    static int getDistance(Rect source, Rect dest, int direction) {
        ...
        int dX, dY; // dest x, y
        switch (direction) {
        case GLView.FOCUS_RIGHT:
            sX = source.right;
            sY = source.top + source.height() / 2;
            dX = dest.left;
            dY = dest.top + dest.height() / 2;
            break;
        case GLView.FOCUS_DOWN:
            sX = source.left + source.width() / 2;
            sY = source.bottom;
            dX = dest.left + dest.width() / 2;
            dY = dest.top;
            break;
        case GLView.FOCUS_LEFT:
            sX = source.left;
            sY = source.top + source.height() / 2;
            dX = dest.right;
            dY = dest.top + dest.height() / 2;
            break;
        case GLView.FOCUS_UP:
            sX = source.left + source.width() / 2;
            sY = source.top;
            dX = dest.left + dest.width() / 2;
            dY = dest.bottom;
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new GLAbsListView.LayoutParams(getContext(), attrs);
    }

    @Override
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        return p instanceof GLAbsListView.LayoutParams;
    }
    ...
    /**
     * Move all views (excluding headers and footers) held by this GLAbsListView into the supplied
     * List. This includes views displayed on the screen as well as views stored in GLAbsListView's
     * internal view recycler.
     *
     * @param views A list into which to put the reclaimed views
     */
    public void reclaimViews(List<GLView> views) {
        int childCount = getChildCount();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            GLAbsListView.LayoutParams lp = (GLAbsListView.LayoutParams) child.getLayoutParams();
	...
	private boolean checkScrap(ArrayList<GLView> scrap) {
		if (scrap == null) {
			return true;
		}
        boolean result = true;
        ...
        for (int i = 0; i < count; i++) {
            final GLView view = scrap.get(i);
            if (view.getGLParent() != null) {
                result = false;
                Log.d("GLAbsListView", "GLAbsListView " + this +
     ...
     * @see android.widget.GLAbsListView.RecyclerListener
     */
    ...
    /**
     * GLAbsListView extends LayoutParams to provide a place to hold the view type.
     */
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        int viewType;						// CHECKSTYLE IGNORE

        /**
         * When this boolean is set, the view has been added to the GLAbsListView
         * at least once. It is used to know whether headers/footers have already
         * been added to the list view and whether they should be treated as
         * recycled views or not.
         */
        @ViewDebug.ExportedProperty
        boolean recycledHeaderFooter;		// CHECKSTYLE IGNORE

        /**
         * When an GLAbsListView is measured with an AT_MOST measure spec, it needs
         ...
         */
        @ViewDebug.ExportedProperty
        boolean forceAdd;					// CHECKSTYLE IGNORE
    ...
    class RecycleBin {
        ...
        /**
         * Unsorted views that can be used by the adapter as a convert GLView.
         */
        ...
        /**
         * Fill ActiveViews with all of the children of the GLAbsListView.
         *
         * @param childCount The minimum number of views mActiveViews should hold
         * @param firstActivePosition The position of the first GLView that will be stored in
         *        mActiveViews
         */
        ...
        /**
         * Get the GLView corresponding to the specified position. The GLView will be removed from
         * mActiveViews if it is found.
         *
         * @param position The position to look up in mActiveViews
         * @return The GLView if it is found, null otherwise
         */
        ...
        /**
         * @return A GLView from the ScrapViews collection. These are unordered.
         */
        ...
        /**
         * Put a GLView into the ScapViews list. These views are unordered.
         *
         * @param scrap The GLView to add
         */
        void addScrapView(GLView scrap) {
            GLAbsListView.LayoutParams lp = (GLAbsListView.LayoutParams) scrap.getLayoutParams();
            if (lp == null) {
                return;
            }
        ...
        void scrapActiveViews() {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (victim != null) {
                    int whichScrap = ((GLAbsListView.LayoutParams) victim.getLayoutParams()).viewType;
