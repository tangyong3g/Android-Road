...
class ExpandableListPosition {
    ...
    long getPackedPosition() {
        if (type == CHILD) return GLExpandableListView.getPackedPositionForChild(groupPos, childPos);
        else return GLExpandableListView.getPackedPositionForGroup(groupPos);
    }
    ...
    static ExpandableListPosition obtainPosition(long packedPosition) {
        if (packedPosition == GLExpandableListView.PACKED_POSITION_VALUE_NULL) {
            return null;
        }
        
        ExpandableListPosition elp = getRecycledOrCreate(); 
        elp.groupPos = GLExpandableListView.getPackedPositionGroup(packedPosition);
        if (GLExpandableListView.getPackedPositionType(packedPosition) ==
                GLExpandableListView.PACKED_POSITION_TYPE_CHILD) {
            elp.type = CHILD;
            elp.childPos = GLExpandableListView.getPackedPositionChild(packedPosition);
        } else {
            elp.type = GROUP;
        }
        return elp;
