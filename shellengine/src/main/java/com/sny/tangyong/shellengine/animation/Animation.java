...
import com.sny.tangyong.shellengine.animator.ValueAnimator;
...
public abstract class Animation implements Cloneable {
    ...
    Transformation3D mPreviousTransformation = new Transformation3D();
    
    long mCurrentTime;
    float mNormalizeTime;
    boolean mToReverse;
    
    private GLView mAnimateView;
...
//        a.recycle();
    ...
    protected Animation clone() throws CloneNotSupportedException {
        ...
        animation.mTransformation = new Transformation3D();
        animation.mPreviousTransformation = new Transformation3D();
        return animation;
    }
    ...
    /**
     * 以当前播放顺序的相反顺序来播放
     * <br>注意：这个“相反”是个相对的概念，相对于当前。
     * <br>另外，一般还需要调用{@link #setFillAfter(boolean)}以保持在动画结束位置。
     */
    public void reverse(GLView view) {
    	mToReverse = true;
    	long startTime = -1;
    	boolean cycleFlip = mCycleFlip;
        if (mStarted && !mEnded) {
            long currentTime = mCurrentTime;
            long currentPlayTime = currentTime - mStartTime;
            long timeLeft = mDuration - currentPlayTime;
            startTime = currentTime - timeLeft;
		} else if (view != null) {
     ...
     */
    public boolean getTransformation(long currentTime, Transformation3D outTransformation) {
...
//                Log.i("Animation", "mRepeated:"+mRepeated);
            } else {
                if (mRepeatCount > 0) {
                    mRepeated++;
                }
    ...
    public void getInvalidateRegion(int left, int top, int right, int bottom,
            RectF invalidate, Transformation3D transformation) {

    	/*
    	 * 获取下一次绘制前需要更新的区域：
    	 * mPreviousRegion 和 mPreviousTransformation 是本次绘制前，视图的区域和变换，
    	 * 通过 transformation 对视图本身区域作变换后再求包围盒，结果放到 invalidate 中，
    	 * 那么 invalidate 和 transformation 会更新到 mPreviousRegion 和 
    	 * mPreviousTransformation。而外部需要获得 mPreviousRegion 和 invalidate 的
    	 * 并集，来通知根视图在下一次绘制之前需要更新该区域。
    	 * mRegion 和 mTransformation 则作为临时变量了。
    	 * 实际上，本次绘制的内容其实使用的是上次开始绘制的时刻的变换。
    	 */
        final RectF tempRegion = mRegion;
        final RectF previousRegion = mPreviousRegion;

        invalidate.set(left, top, right, bottom);
        transformation.mapRect(invalidate);
...
//        previousRegion.set(tempRegion);
        //使用交换引用的方法避免复制赋值
        mRegion = mPreviousRegion;
        mPreviousRegion = tempRegion;

        final Transformation3D tempTransformation = mTransformation;
...
//        previousTransformation.set(tempTransformation);
        //使用交换引用的方法避免复制赋值
        mTransformation = mPreviousTransformation;
        mPreviousTransformation = tempTransformation;
    ...
    public void initializeInvalidateRegion(int left, int top, int right, int bottom) {
        ...
        if (mFillBefore) {
            final Transformation3D previousTransformation = mPreviousTransformation;
            applyTransformation(mInterpolator.getInterpolation(mNormalizeTime), previousTransformation);
        }
    }
    ...
    public void setAnimateView(GLView view) {
    	mAnimateView = view;
    }
