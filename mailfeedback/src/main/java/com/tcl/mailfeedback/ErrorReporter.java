...
//			preferences.setPreferences(context, Context.MODE_PRIVATE, IPreferencesIds.LANGUAGE_SETTING_NAME);
...
//			mCrashProperties.put(ADVANCED_SETTING_LANGUAGE, settingLanguage); // 高级设置语言
	...
	private String saveCrashReportFile() {
		try {
			...
			} else if ((track.contains(OUT_OF_MEMORY_FITER_EA) || track
					.contains(OUT_OF_MEMORY_FITER_EB))
					&& (track.contains(OUT_OF_MEMORY_FITER_A) || track
							.contains(OUT_OF_MEMORY_FITER_B))) {
				// mIsOutOfMemoryError = true;
			}
