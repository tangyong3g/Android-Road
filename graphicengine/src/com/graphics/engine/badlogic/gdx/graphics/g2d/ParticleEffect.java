...
public class ParticleEffect implements Disposable {
	...
	public ParticleEffect () {
		emitters = new ArrayList<ParticleEmitter>(8);
	}

	public ParticleEffect (ParticleEffect effect) {
		emitters = new ArrayList<ParticleEmitter>(effect.emitters.size());
	...
	public void load (Context context, String effectFile) {
		loadEmitters(getInputStream(context.getResources(), effectFile), effectFile);
...
//	}
	...
	private static InputStream getInputStream(Resources res, String fileName) {
		try {
	...
	public void loadEmitters (InputStream input, String effectFile) {
		emitters.clear();
...
//	}
	...
	public void loadEmitterImages (Resources res, String imagesDir) {
		for (int i = 0, n = emitters.size(); i < n; i++) {
			ParticleEmitter emitter = emitters.get(i);
			String imagePath = emitter.getImagePath();
			if (imagePath == null) continue;
...
//			emitter.setSprite(new Sprite(loadTexture(imagesDir.child(imageName))));
			...
			int id = res.getIdentifier(imagePath, "drawable", imagesDir);
			GLDrawable drawable = GLDrawable.getDrawable(res, id);
			if (drawable != null) {
				emitter.setSprite(new Sprite(drawable.getTexture()));
				//TODO:清除drawable
			}
		}
	}
	...
	public void dispose () {
		for (int i = 0, n = emitters.size(); i < n; i++) {
			ParticleEmitter emitter = emitters.get(i);
			emitter.getSprite().getTexture().clear();
		}
	}
}
