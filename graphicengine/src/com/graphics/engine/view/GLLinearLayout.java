...
public class GLLinearLayout extends GLViewGroup {
    ...
    public GLLinearLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, com_android_internal_R_styleable.LinearLayout);

        int index = a.getInt(com_android_internal_R_styleable.LinearLayout_orientation, -1);
        if (index >= 0) {
            setOrientation(index);
        }

        index = a.getInt(com_android_internal_R_styleable.LinearLayout_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }

        boolean baselineAligned = a.getBoolean(com_android_internal_R_styleable.LinearLayout_baselineAligned, true);
        if (!baselineAligned) {
            setBaselineAligned(baselineAligned);
        }

        mWeightSum = a.getFloat(com_android_internal_R_styleable.LinearLayout_weightSum, -1.0f);

        mBaselineAlignedChildIndex =
                a.getInt(com_android_internal_R_styleable.LinearLayout_baselineAlignedChildIndex, -1);

        // TODO: Better name, add Java APIs, make it public
        mUseLargestChild = a.getBoolean(com_android_internal_R_styleable.LinearLayout_useLargestChild, false);
...
//    @android.view.RemotableViewMethod
    ...
    public int getBaseline() {
        ...
        final GLView child = getChildAt(mBaselineAlignedChildIndex);
...
//    @android.view.RemotableViewMethod
     ...
     */
    GLView getVirtualChildAt(int index) {
...
//    @android.view.RemotableViewMethod
    ...
    void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; ++i) {
            final GLView child = getVirtualChildAt(i);
     ...
     */
    int getChildrenSkipCount(GLView child, int index) {
        return 0;
    }
     ...
     */
    void measureChildBeforeLayout(GLView child, int childIndex,
     ...
     */
    int getLocationOffset(GLView child) {
        return 0;
    }
     ...
     */
    int getNextLocationOffset(GLView child) {
        return 0;
    }
    ...
    private void setChildFrame(GLView child, int left, int top, int width, int height) {        
