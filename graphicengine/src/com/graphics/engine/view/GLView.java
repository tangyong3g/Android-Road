...
import com.graphics.engine.ICleanup;
...
import com.graphics.engine.animator.ValueAnimator.AnimatorUpdateListener;
...
import com.graphics.engine.graphics.GLFramebuffer;
import com.graphics.engine.graphics.RenderInfoNode;
...
import com.graphics.engine.widget.ScrollBarDrawable;
...
public class GLView implements Drawable.Callback, KeyEvent.Callback, AccessibilityEventSource, ICleanup {
    ...
    */
    ...
    protected Drawable mBGDrawable = null;
    ...
    /**
     * {@link #setDrawingCacheInvalidatable(boolean)}
     */
    boolean mDrawingCacheInvalidatable = true;  // 是否禁用来自invalidate的绘图缓冲刷行
...
//        }
    ...
    /**
     * 设置绘图缓冲是否会可以响应来自invalidate的更新
     * 设置为true时，本view的更行不会会知道绘图缓冲上，可以减少绘制，提高帧率，但是内容不会变化
     * 设置为false时，绘图缓冲会响应invalidate时修改的标志位在buildDrawingCache中更新绘制，会减低帧率，但是绘图缓冲可以随view内容变化更新
     * @param invalidatable
     */
    public void setDrawingCacheInvalidatable(boolean invalidatable) {
    	mDrawingCacheInvalidatable = invalidatable;
    }
...
//			mDrawingCache.unregister();
    ...
    public void buildDrawingCache(GLCanvas canvas) {
    	...
    	boolean refresh = (mPrivateFlags & DRAWING_CACHE_VALID) == 0 && (mDrawingCacheInvalidatable || mDrawingCache == null);
        ...
        if (refresh) {
			...
			if (mDrawingCache == null) {
				if (mDrawingCacheScale != null) {
					...
					mDrawingCache = new GLFramebuffer(scaledWidth, scaledHeight, true, mDrawingCacheDepthBufferBits, 0, false);
				} else {
					mDrawingCache = new GLFramebuffer(width, height, true, mDrawingCacheDepthBufferBits, 0, false);
				}
