...
import com.graphics.engine.util.LongSparseArray;
import com.graphics.engine.view.GLView;
import com.graphics.engine.view.GLViewParent;
import com.graphics.engine.view.com_android_internal_R_styleable;

/**
 * 
 * <br>类描述: LitView for ShellEngine
 * <br>功能详细描述:
 * 
 */
public class GLListView extends GLAbsListView {
    ...
    public class FixedViewInfo {	// CHECKSTYLE IGNORE
        public GLView view;
        public Object data;
        public boolean isSelectable;
    }

    private ArrayList<FixedViewInfo> mHeaderViewInfos = new ArrayList<GLListView.FixedViewInfo>();
    private ArrayList<FixedViewInfo> mFooterViewInfos = new ArrayList<GLListView.FixedViewInfo>();
...
//    private Paint mDividerPaint;
    ...
    public GLListView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.obtainStyledAttributes(attrs,
                com_android_internal_R_styleable.ListView, defStyle, 0);

        CharSequence[] entries = a.getTextArray(
        		com_android_internal_R_styleable.ListView_entries);
        //TODO
        if (entries != null) {
//            setAdapter(new GLListAdapter(context,
//                    com.android.internal.R.layout.simple_list_item_1, entries));
        }

        final Drawable d = a.getDrawable(com_android_internal_R_styleable.ListView_divider);
        if (d != null) {
        ...
        final int dividerHeight = a.getDimensionPixelSize(
        		com_android_internal_R_styleable.ListView_dividerHeight, 0);
        if (dividerHeight != 0) {
            setDividerHeight(dividerHeight);
        }

        setChoiceMode(a.getInt(com_android_internal_R_styleable.ListView_choiceMode, CHOICE_MODE_NONE));
        
        mHeaderDividersEnabled = a.getBoolean(com_android_internal_R_styleable.ListView_headerDividersEnabled, true);
        mFooterDividersEnabled = a.getBoolean(com_android_internal_R_styleable.ListView_footerDividersEnabled, true);
    ...
    public void addHeaderView(GLView v, Object data, boolean isSelectable) {
    ...
    public void addHeaderView(GLView v) {
     ...
     *         view
     */
    public boolean removeHeaderView(GLView v) {
        if (mHeaderViewInfos.size() > 0) {
            boolean result = false;
            if (((GLHeaderViewListAdapter) mAdapter).removeHeader(v)) {
    ...
    private void removeFixedViewInfo(GLView v, ArrayList<FixedViewInfo> where) {
        int len = where.size();
        for (int i = 0; i < len; ++i) {
     ...
     */
    public void addFooterView(GLView v, Object data, boolean isSelectable) {
        FixedViewInfo info = new FixedViewInfo();
        info.view = v;
        info.data = data;
        info.isSelectable = isSelectable;
     ...
     * @param v The view to add.
     */
    public void addFooterView(GLView v) {
     ...
     */
    public boolean removeFooterView(GLView v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
			if (((GLHeaderViewListAdapter) mAdapter).removeFooter(v)) {
                mDataSetObserver.onChanged();
                result = true;
            }
    ...
    public void setAdapter(GLListAdapter adapter) {
		...
		if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
            mAdapter =  new GLHeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
        } else {
            mAdapter = adapter;
        }
    ...
    private void clearRecycledState(ArrayList<FixedViewInfo> infos) {
        if (infos != null) {
            ...
            for (int i = 0; i < count; i++) {
                final GLView child = infos.get(i).view;
    ...
    public boolean requestChildRectangleOnScreen(GLView child, Rect rect, boolean immediate) {
     ...
     *         range that we draw.
     */
    private GLView fillDown(int pos, int nextTop) {
        GLView selectedView = null;
        ...
        while (nextTop < end && pos < mItemCount) {
            ...
            GLView child = makeAndAddView(pos, nextTop, true, mListPadding.left, selected);
     ...
     */
    private GLView fillUp(int pos, int nextBottom) {
        GLView selectedView = null;
        ...
        while (nextBottom > end && pos >= 0) {
            // is this the selected item?
            boolean selected = pos == mSelectedPosition;
            GLView child = makeAndAddView(pos, nextBottom, false, mListPadding.left, selected);
     ...
     * @return The view that is currently selected
     */
    private GLView fillFromTop(int nextTop) {
     ...
     */
    private GLView fillFromMiddle(int childrenTop, int childrenBottom) {
        int height = childrenBottom - childrenTop;

        int position = reconcileSelectedPosition();

        GLView sel = makeAndAddView(position, childrenTop, true,
     ...
     */
    private void fillAboveAndBelow(GLView sel, int position) {
     ...
     */
    private GLView fillFromSelection(int selectedTop, int childrenTop, int childrenBottom) {
     ...
     * @return The view that currently has selection
     */
    private GLView moveSelection(GLView oldSel, GLView newSel, int delta, int childrenTop,
            int childrenBottom) {
        int fadingEdgeLength = getVerticalFadingEdgeLength();
        final int selectedPosition = mSelectedPosition;
        ...
        if (delta > 0) {
            ...
            if (oldTop < childrenTop) {
				...
				if (newBottom < childrenTop + 20) { // CHECKSTYLE IGNORE
	...
	private class FocusSelector implements Runnable { // CHECKSTYLE IGNORE
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mItemCount > 0 && (widthMode == MeasureSpec.UNSPECIFIED ||
            ...
            final GLView child = obtainView(0, mIsScrap);
    ...
    private void measureScrapChild(GLView child, int position, int widthMeasureSpec) {
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition,
        ...
        final GLListAdapter adapter = mAdapter;
        if (adapter == null) {
     ...
     *         visible area.
     */
    private GLView fillSpecific(int position, int top) {
        boolean tempIsSelected = position == mSelectedPosition;
        GLView temp = makeAndAddView(position, top, true, mListPadding.left, tempIsSelected);
    ...
    private void correctTooHigh(int childCount) {
        ...
        if (lastPosition == mItemCount - 1 && childCount > 0) {
            ...
            // Get the last child ...
            final GLView lastChild = getChildAt(childCount - 1);
            ...
            GLView firstChild = getChildAt(0);
    ...
    private void correctTooLow(int childCount) {
        ...
        if (mFirstPosition == 0 && childCount > 0) {
            ...
            // Get the first child ...
            final GLView firstChild = getChildAt(0);
            ...
            GLView lastChild = getChildAt(childCount - 1);
            final int lastBottom = lastChild.getBottom();
            int lastPosition = mFirstPosition + childCount - 1;
    ...
    protected void layoutChildren() {
        ...
        try {
            ...
            GLView sel;
            GLView oldSel = null;
            GLView oldFirst = null;
            GLView newSel = null;

            GLView focusLayoutRestoreView = null;
            ...
            GLView focusLayoutRestoreDirectChild = null;
...
//                    }
            ...
            // to someone else
            final GLView focusedChild = getFocusedChild();
            if (focusedChild != null) {
            ...
            if (sel != null) {
                ...
                if (mItemsCanFocus && hasFocus() && !sel.hasFocus()) {
                    ...
                    if (!focusWasTaken) {
                        ...
                        // has focus
                        final GLView focused = getFocusedChild();
                        if (focused != null) {
                            focused.clearFocus();
                        }
                ...
                if (mTouchMode > TOUCH_MODE_DOWN && mTouchMode < TOUCH_MODE_SCROLL) {
                    GLView child = getChildAt(mMotionPosition - mFirstPosition);
					if (child != null) {
						positionSelector(child);
					}
                } else {
    ...
    /**
     * @param focusedChild a direct child of this list.
     * @return Whether child is a header or footer view.
     */
    private boolean isDirectChildHeaderOrFooter(GLView focusedChild) {

        final ArrayList<FixedViewInfo> headers = mHeaderViewInfos;
        final int numHeaders = headers.size();
        for (int i = 0; i < numHeaders; i++) {
            if (focusedChild == headers.get(i).view) {
                return true;
            }
        }
        ...
        for (int i = 0; i < numFooters; i++) {
            if (focusedChild == footers.get(i).view) {
                return true;
            }
        }
        return false;
     ...
     */
    private GLView makeAndAddView(int position, int y, boolean flow, int childrenLeft,
            boolean selected) {
        GLView child;
...
//                            position, getChildCount());
     ...
     */
    private void setupChild(GLView child, int position, int y, boolean flowDown, int childrenLeft,
        ...
        GLAbsListView.LayoutParams p = (GLAbsListView.LayoutParams) child.getLayoutParams();
        
        if (p == null) {
            p = new GLAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        p.viewType = mAdapter.getItemViewType(position);
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (!handled) {
            ...
            GLView focused = getFocusedChild();
...
//            final GLView selectedView = getSelectedView();
//            if (selectedView != null && selectedView.hasFocus() &&
//                    selectedView instanceof GLViewGroup) {
//
//                final GLView currentFocus = selectedView.findFocus();
//                final GLView nextFocus = FocusFinder.getInstance().findNextFocus(
//                        (GLViewGroup) selectedView, currentFocus, direction);
//                if (nextFocus != null) {
...
//                final GLView globalNextFocus = FocusFinder.getInstance().findNextFocusFromRect(
//                        (ViewGroup) getRootView(), currentFocus, direction);
//                if (globalNextFocus != null) {
//                    return isViewAncestorOf((GLViewParent) globalNextFocus, this);
//                }
//            }
//        }
        return false;
    ...
    private boolean arrowScrollImpl(int direction) {
        ...
        if (mItemsCanFocus && (focusResult == null)
            ...
            final GLView focused = selectedView.findFocus();
        ...
        if (nextSelectedPosition == INVALID_POSITION && selectedView != null
                && !isViewAncestorOf((GLViewParent) selectedView, this)) {
            selectedView = null;
            hideSelector();
     ...
     */
    private void handleNewSelectionChange(GLView selectedView, int direction, int newSelectedPosition,
     ...
     * @param topView The child
     * @param childIndex The view group index of the child.
     * @param numChildren The number of children in the view group.
     */
    private void measureAndAdjustDown(GLView topView, int childIndex, int numChildren) {
        int oldHeight = topView.getHeight();
        measureItem(topView);
        if (topView.getMeasuredHeight() != oldHeight) {
            // lay out the view, preserving its top
            relayoutMeasuredItem(topView);

            // adjust views below appropriately
            final int heightDelta = topView.getMeasuredHeight() - oldHeight;
     ...
     * @param topView The child.
     */
    private void measureItem(GLView topView) {
        ...
        topView.measure(childWidthSpec, childHeightSpec);
    }
     ...
     * @param topView The child.
     */
    private void relayoutMeasuredItem(GLView topView) {
        final int w = topView.getMeasuredWidth();
        final int h = topView.getMeasuredHeight();
        final int childLeft = mListPadding.left;
        final int childRight = childLeft + w;
        final int childTop = topView.getTop();
        final int childBottom = childTop + h;
        topView.layout(childLeft, childTop, childRight, childBottom);
    }
    ...
    private int amountToScroll(int direction, int nextSelectedPosition) {
        ...
        if (direction == View.FOCUS_DOWN) {
            ...
            final GLView viewToMakeVisible = getChildAt(indexToMakeVisible);
    ...
    private int lookForSelectablePositionOnScreen(int direction) {
        ...
        if (direction == View.FOCUS_DOWN) {
            ...
            final GLListAdapter adapter = getAdapter();
...
//        GLView newFocus;
//        if (selectedView != null && selectedView.hasFocus()) {
//            GLView oldFocus = selectedView.findFocus();
...
//        }
     ...
     */
    private int positionOfNewFocus(GLView newFocus) {
        final int numChildren = getChildCount();
        for (int i = 0; i < numChildren; i++) {
            final GLListView child = (GLListView) getChildAt(i);
            if (isViewAncestorOf((GLViewParent) newFocus, child)) {
                return mFirstPosition + i;
            }
        }
     ...
     */
    private boolean isViewAncestorOf(GLViewParent theParent2, GLListView glListView) {
        if (theParent2 == glListView) {
            return true;
        }

        final GLViewParent theParent = theParent2.getGLParent();
		return (theParent instanceof ViewGroup) && isViewAncestorOf(theParent, glListView);
    }
     ...
     *   needs to be taken into account when actually scrolling.
     */
    private int amountToScrollToNewFocus(int direction, GLView newFocus, int positionOfNewFocus) {
     ...
     * direction.
     * 
     * @param focused A descendant of this list.
     * @return The distance, or 0 if the nearest edge is already on screen.
     */
    private int distanceToView(GLView focused) {
        int distance = 0;
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            GLView last = getChildAt(numChildren - 1);
            ...
            GLView last = getChildAt(lastIndex);
    ...
    private GLView addViewAbove(GLView first, int position) {
        int abovePosition = position - 1;
        GLView view = obtainView(abovePosition, mIsScrap);
        int edgeOfNewChild = first.getTop() - mDividerHeight;
        setupChild(view, abovePosition, edgeOfNewChild, false, mListPadding.left,
                false, mIsScrap[0]);
        return view;
    }

    private GLView addViewBelow(GLView last, int position) {
        int belowPosition = position + 1;
        GLView view = obtainView(belowPosition, mIsScrap);
        int edgeOfNewChild = last.getBottom() + mDividerHeight;
	...
	public void setCacheColorHint(int color) {
		final boolean opaque = (color >>> 24) == 0xFF;	// CHECKSTYLE IGNORE
...
//            mDividerPaint.setColor(color);
			if (mColorDivider == null) {
				mColorDivider = new ColorGLDrawable(color);
			}
        }
...
//            }
            if (fillForMissingDividers && mColorDivider == null && mIsCacheColorOpaque) {
            	mColorDivider = new ColorGLDrawable(getCacheColorHint());
            }
...
//                                canvas.drawRect(bounds, paint);
                                drawColorDivider(canvas, bounds);
...
//                                canvas.drawRect(bounds, paint);
    ...
    void drawDivider(GLCanvas canvas, Rect bounds, int childIndex) {
        ...
        divider.setBounds(0, 0, bounds.width(), bounds.height());
        
//        divider.draw(canvas);
        canvas.drawDrawable(divider);
        canvas.restore();
    }
    
    void drawColorDivider(GLCanvas canvas, Rect bounds) {
        final Drawable divider = mColorDivider;
        canvas.save();
        canvas.translate(bounds.left, bounds.top);
        divider.setBounds(0, 0, bounds.width(), bounds.height());
...
//        divider.draw(canvas);
     ...
     * 如果是GLDrawable，当cleanup的时候也会clear掉，并且在重新调用本方法时，前一个divider也会被clear掉。
     * 因此，如果不想被clear掉，需要调用{@link GLDrawable#duplicate()} 来增加引用计数。
     */
    public void setDivider(Drawable divider) {
        if (divider != null) {
            mDividerHeight = divider.getIntrinsicHeight();
			if (mDivider instanceof GLDrawable && mDivider != divider) {
				((GLDrawable) mDivider).clear();
			}
        } else {
            mDividerHeight = 0;
        }
     ...
     */
    GLView findViewInHeadersOrFooters(ArrayList<FixedViewInfo> where, int id) {
        if (where != null) {
            int len = where.size();
            GLView v;
     ...
     */
    GLView findViewTagInHeadersOrFooters(ArrayList<FixedViewInfo> where, Object tag) {
        if (where != null) {
            int len = where.size();
            GLView v;
...
//    	Log.i("liuyong","dispatchTouchEvent");
//    	return true;
//    }
    /**
        ...
        if (mChoiceMode != CHOICE_MODE_NONE) {
            ...
            if (mChoiceMode == CHOICE_MODE_MULTIPLE) {
                ...
                if (mCheckedIdStates != null && mAdapter.hasStableIds()) {
                    if (newValue) {
                        mCheckedIdStates.put((int) mAdapter.getItemId(position), Boolean.TRUE);
                    } else {
	...
	static class SavedState extends BaseSavedState {		// CHECKSTYLE IGNORE
		SparseBooleanArray checkState;						// CHECKSTYLE IGNORE
		LongSparseArray<Boolean> checkIdState;				// CHECKSTYLE IGNORE
...
//    }
	...
	public void cleanup() {
		if (mDivider != null) {
			if (mDivider instanceof GLDrawable) {
				((GLDrawable) mDivider).clear();
			} else {
				releaseDrawableReference(mDivider);
			}
			mDivider = null;
		}
		if (mColorDivider != null) {
