...
class GLExpandableListConnector extends GLBaseAdapter implements GLFilterable {
     ...
     */
    private GLExpandableListAdapter mExpandableListAdapter;
     ...
     */
    public GLExpandableListConnector(GLExpandableListAdapter expandableListAdapter) {
     ...
     */
    public void setExpandableListAdapter(GLExpandableListAdapter expandableListAdapter) {
    ...
    @Override
    public GLView getView(int flatListPos, GLView convertView, GLViewGroup parent) {
        final PositionMetadata posMetadata = getUnflattenedPos(flatListPos);
    ...
    public int getItemViewType(int flatListPos) {
        ...
        int retValue;
        if (mExpandableListAdapter instanceof GLHeterogeneousExpandableList) {
            GLHeterogeneousExpandableList adapter =
                    (GLHeterogeneousExpandableList) mExpandableListAdapter;
            if (pos.type == ExpandableListPosition.GROUP) {
        ...
        if (syncGroupPositions) {
            ...
            for (int i = egmlSize - 1; i >= 0; i--) {
                ...
                if (newGPos != curGm.gPos) {
                    if (newGPos == GLAdapterView.INVALID_POSITION) {
    ...
    GLExpandableListAdapter getAdapter() {
        return mExpandableListAdapter;
    }
    
    public GLFilter getFilter() {
        GLExpandableListAdapter adapter = getAdapter();
        if (adapter instanceof GLFilterable) {
            return ((GLFilterable) adapter).getFilter();
        } else {
            return null;
        }
    }
    ...
    int findGroupPosition(long groupIdToMatch, int seedGroupPosition) {
        ...
        if (count == 0) {
            return GLAdapterView.INVALID_POSITION;
        }

        // If there isn't a selection don't hunt for it
        if (groupIdToMatch == GLAdapterView.INVALID_ROW_ID) {
            return GLAdapterView.INVALID_POSITION;
        }
        ...
        long endTime = SystemClock.uptimeMillis() + GLAdapterView.SYNC_MAX_DURATION_MILLIS;
