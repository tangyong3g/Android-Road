...
public abstract class BaseConnectHandle {
    ...
    // "http://61.145.124.212:8082/";
    // private static final String POST_DATA_DEBUG_URL =
    // "http://192.168.214.120:8080/GOClientData/DR?ptl=10&is_zip=1";
    ...
    // public static String TEST_UPLOADURL =
    // "http://61.145.124.212:8082/GOClientData/DR?ptl=10&is_zip=1";
    // public static String TEST_CHANNEL_CONTROL_URL =
    // "http://61.145.124.212:8082/GOClientData/ComCtrl";
    ...
    public void postData(PostBean bean) {
        if (BaseConnectHandle.RET_ERRO_MALFORMEDURLEXCEPTION == prepareConnect(bean.mFunId, bean.mData)) {// 外界传入错误的url直接忽略该条数据
        ...
        try {
            ...
            if (StatisticsManager.getInstance(mContext).getDebugMode()) {
                ...
                if (funid == StatisticsManager.BASIC_FUN_ID) {
                    url = new URL(BaseConnectHandle.UPLOAD_URL + BaseConnectHandle.BASIC_CONTROL);
                } else {
